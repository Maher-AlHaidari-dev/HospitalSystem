document.addEventListener('DOMContentLoaded', () => {
    ...
    // تطبيق اللغة المحفوظة قبل تحميل البيانات
    ...
    /*
     * مهم:
     * لا يوجد onclick داخل HTML.
     * لذلك نربط الزر مرة واحدة فقط هنا.
     */
...
function handleAuthError(error) {
    ...
    if (
        ...
        alert(
            'انتهت صلاحية الجلسة أو لم يتم تسجيل الدخول، يرجى تسجيل الدخول من جديد.'
        );
...
async function loadAppointments() {
    ...
    const dict =
        CONFIG.translations[currentLang] ||
        CONFIG.translations['ar'];
    ...
    const tdLoading = CONFIG.createSafeElement(
        'td',
        dict.loadingAppointments,
        'py-8 text-center text-slate-400 font-medium'
    );

    tdLoading.setAttribute(
        'data-i18n',
        'loadingAppointments'
    );
    ...
    try {
        ...
        const appointments = await CONFIG.request(
            'api/Appointments',
            {
        ...
        const tdError = CONFIG.createSafeElement(
            'td',
            dict.errorFetchAppointments,
            'py-8 text-center text-rose-500 font-bold'
        );

        tdError.setAttribute(
            'data-i18n',
...
function renderAppointmentsTable(appointments) {
    ...
    if (!appointments || appointments.length === 0) {
        ...
        const tdEmpty = CONFIG.createSafeElement(
            'td',
            dict.noAppointmentsFound,
            'py-8 text-center text-slate-400 font-medium'
        );

        tdEmpty.setAttribute(
            'data-i18n',
            'noAppointmentsFound'
        );
    ...
    appointments.forEach((item, index) => {
        ...
        tr.appendChild(
            CONFIG.createSafeElement(
                'td',
                (index + 1).toString(),
                'py-3.5 px-3 font-mono text-[11px] text-slate-500'
            )
        );
        ...
        statusTd.appendChild(
            CONFIG.createSafeElement(
                'span',
                translatedStatus,
                `inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${statusClasses}`
            )
        );
